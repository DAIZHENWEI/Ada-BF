...
(for Ada-BF and disjoint Ada-BF) `--num_group_min` and `--num_group_max` give the range of number of groups to divide (range of <img src="https://render.githubusercontent.com/render/math?math=g">
); `--c_min` and `--c_max` provide the range of $$c$$ where $$c = \frac{m_j}{m_{j+1}}$$
